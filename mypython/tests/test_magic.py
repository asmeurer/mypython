...
def test_timeit():
    ...
    out, err = _test_output('%timeit time.sleep(1)\n', _globals=_globals,
        _locals=_locals, remove_terminal_sequences=True)
...
def test_doctest():
    ...
    try:
        _globals = _test_globals.copy()
        _locals = _test_locals.copy()
        assert _test_output('%doctest\n', _globals=_globals, _locals=_locals) == ('doctest mode enabled\n', '')
...
def test_debug():
    ...
    try:
        _globals = _test_globals.copy()
        _locals = _test_locals.copy()
        assert _test_output('%debug\n', _globals=_globals, _locals=_locals) == ('mypython debugging mode enabled\n\n', '')
        assert mypython.DEBUG
        assert _test_output('%debug\n', _globals=_globals, _locals=_locals) == ('mypython debugging mode disabled\n\n', '')
...
def test_sympy():
    _globals = _test_globals.copy()
    _locals = _test_locals.copy()
    assert _test_output('%sympy\n', _globals=_globals, _locals=_locals) == (textwrap.indent(sympy_start, '    ') + '\n\n', '')
